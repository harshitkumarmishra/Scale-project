...
def dram_trace_read_v2(
    ...
    with open(sram_trace_file, 'r') as sram_requests:
...
def dram_trace_write(ofmap_sram_size = 64,
    ...
    with open(dram_write_trace_file, 'w') as trace_file:
        last_clk = 0
        clk = 0
        ...
        for row in traffic:
            ...
            if (
                len(sram_buffer[filling_buf])
                + (len(elems) - 1) * data_width_bytes
                >= ofmap_sram_size
            ):
                ...
                draining_buf, filling_buf = filling_buf, draining_buf
